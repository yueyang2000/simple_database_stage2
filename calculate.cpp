...
map<string, int> calculate::p = { {"+",4},{"-",4},{"*",5},{"/",5},{"%",5},{"DIV",5},{"MOD",5}, {"(",0},{"ABS(",0},{"SIN(",0},{"COS(",0},{"FLOOR(",0}
,{"TAN(",0},{"SQRT(",0},{")",0} ,{"OR",1},{"XOR",1},{"AND",2},{"NOT",3} };
...
string calculate::getresult() {
	...
	while (ss >> piece) {
		if (isnumber(piece)) {//如果是数字直接入栈
			...
			if (piece != "NOT"&&!isfuc(piece)) {//二元运算符
...
void calculate::form() {
	...
	r = " ?( (OR)| (XOR)|(AND)|(NOT)) ?";
	expression = regex_replace(expression, r, " $1 ");
	r = " ?((ABS) | (SIN) | (EXP) | (COS) | (TAN) | (SQRT) | (FLOOR)) ?";
	expression = regex_replace(expression, r, " $1 ");
